...
class FilenSDK {
    ...
    constructor(params, workers, axiosInstance) {
        ...
        this._locks = {
            ...
            intensive: new lock_1.default({
                sdk: this,
                resource: "intensive"
            }),
            auth: new lock_1.default({
                sdk: this,
                resource: "auth"
            })
        };
            ...
            if (authVersion === 1 || authVersion === 2) {
                ...
                for (const masterKey of masterKeys) {
                    try {
                        ...
                        if (typeof decryptedMasterKeys === "string" &&
    ...
    async login({ email, password, twoFactorCode }) {
        ...
        this.init(Object.assign(Object.assign({}, this.config), { email: emailToUse, password: "redacted", twoFactorCode: "redacted", apiKey: loginResponse.apiKey, authVersion }));
        await this._locks.auth.acquire();
        try {
            ...
            this.init(Object.assign(Object.assign({}, this.config), { email: emailToUse, password: "redacted", twoFactorCode: "redacted", masterKeys: updateKeys.masterKeys, apiKey: loginResponse.apiKey, publicKey: updateKeys.publicKey, privateKey: updateKeys.privateKey, authVersion, baseFolderUUID: baseFolderResponse.uuid, userId: infoResponse.id }));
        }
        finally {
            await this._locks.auth.release().catch(() => { });
        }
    }
    async register({ email, password, refId, affId }) {
        let salt = "";
        let derived = {
            derivedMasterKeys: "",
            derivedPassword: ""
        };
        if (constants_1.REGISTER_AUTH_VERSION === 3) {
            ...
            derived = await this.getWorker().crypto.utils.generatePasswordAndMasterKeyBasedOnAuthVersion({
                ...
                salt,
                authVersion: constants_1.REGISTER_AUTH_VERSION
            });
        }
        else {
        ...
        await this.api(3).register(Object.assign(Object.assign({ email, password: derived.derivedPassword, salt, authVersion: constants_1.REGISTER_AUTH_VERSION }, (typeof refId === "string" && refId.length > 0 && refId.length < 128 ? { refId } : {})), (typeof affId === "string" && affId.length > 0 && affId.length < 128 ? { affId } : {})));
    }
    /**
     * Logout.
    ...
    async clearTemporaryDirectory() {
        ...
        await utils_1.default.clearTempDirectory({
            tmpDir
        });
    }
}
