...
class Cloud {
    ...
    async editFileMetadata({ uuid, metadata }) {
        ...
        if (!(0, utils_1.isValidFileName)(metadata.name)) {
    ...
    async renameFile({ uuid, metadata, name, overwriteIfExists = false }) {
        ...
        if (!(0, utils_1.isValidFileName)(name)) {
    ...
    async filePublicLinkInfo({ uuid, password, salt, key }) {
        ...
        const derivedPassword = password
            ? salt && salt.length === 512
                ? Buffer.from(await this.sdk.getWorker().crypto.utils.argon2id(Buffer.from(password, "utf-8"), Buffer.from(salt, "hex"), {
                    t: 3,
                    m: 65536,
                    p: 4,
                    version: 0x13,
        ...
        try {
            ...
            return Object.assign(Object.assign({}, info), { name: nameDecrypted.length > 0 ? nameDecrypted : `CANNOT_DECRYPT_NAME_${uuid}`, mime: nameDecrypted.length > 0 ? mimeDecrypted : "application/octet-stream", size: nameDecrypted.length > 0 ? parseInt(sizeDecrypted) : 0 });
        }
        catch (_a) {
            return Object.assign(Object.assign({}, info), { name: `CANNOT_DECRYPT_NAME_${uuid}`, mime: "application/octet-stream", size: 0 });
        }
    }
    /**
        ...
        try {
            ...
            if (size > 0) {
                ...
                await new Promise((resolve, reject) => {
                    (0, stream_1.pipeline)(stream_1.Readable.fromWeb(readStream), fs_extra_1.default.createWriteStream(destinationPath), err => {
                        if (err) {
                            reject(err);
                            return;
                        }
                ...
                await fs_extra_1.default.writeFile(destinationPath, Buffer.from([]));
            }
        ...
        for (const folder of contents.folders) {
            try {
                ...
                tree[folderPath] = {
                    ...
                    favorited: (_a = folder.favorited) !== null && _a !== void 0 ? _a : false,
                    color: (_b = folder.color) !== null && _b !== void 0 ? _b : null
                };
            }
            catch (_c) {
                continue;
            ...
            promises.push(new Promise((resolve, reject) => {
                ...
                decryptPromise
                    ...
                    tree[filePath] = {
                        ...
                        favorited: (_a = file.favorited) !== null && _a !== void 0 ? _a : false
                    };
                    resolve();
                })
                    .catch(reject);
    ...
    async downloadDirectoryToLocal({ uuid, type = "normal", linkUUID, linkHasPassword, linkPassword, linkSalt, to, abortSignal, pauseSignal, onQueued, onStarted, onError, onFinished, onProgress, onProgressId, linkKey }) {
        ...
        try {
            ...
            for (const path in tree) {
                ...
                // Decrypted item names are user-supplied and can contain separators or ".." segments. Tree keys are built with
                // posix.join (which ignores "\"), so on Windows a backslash name survives into the key and this pathModule.join
                // (win32) can re-resolve it outside destinationPath. Skip any entry whose resolved target lands outside destination.
                const resolvedDestination = path_1.default.resolve(destinationPath);
                const resolvedFilePath = path_1.default.resolve(filePath);
                if (resolvedFilePath !== resolvedDestination && !resolvedFilePath.startsWith(resolvedDestination + path_1.default.sep)) {
                    continue;
                }
                promises.push(new Promise((resolve, reject) => {
     ...
     * @returns {Promise<CloudItem>}
     */
    async uploadLocalFile({ source, parent, name, pauseSignal, abortSignal, onProgress, onProgressId, onQueued, onStarted, onError, onFinished, onUploaded, uuid, encryptionKey }) {
        if (constants_1.environment !== "node") {
        ...
        try {
            ...
            const fileHasher = constants_1.environment === "node" || constants_1.environment === "react-native" ? crypto_1.default.createHash("sha512") : sha512_1.sha512.create();
            if (fileSize > 0) {
                const waitForPause = async () => {
            ...
            const [nameEncrypted, mimeEncrypted, sizeEncrypted, metadata, nameHashed] = await Promise.all([
                ...
                this.sdk.getWorker().crypto.encrypt.metadata({
                    metadata: JSON.stringify({
                        ...
                        lastModified,
                        creation,
                        hash: Buffer.from(fileHasher.digest()).toString("hex")
                    })
                }),
                this.sdk.getWorker().crypto.utils.hashFileName({
                    name: fileName,
            ...
            try {
                const done = fileSize > 0
                    ? await this.sdk.api(3).upload().done({
                        ...
                        rm,
                        metadata,
                        version: constants_1.FILE_ENCRYPTION_VERSION,
                        uploadKey
                    })
     ...
     * @returns {Promise<CloudItem>}
     */
    async uploadLocalFileStream({ source, parent, name, pauseSignal, abortSignal, onProgress, onProgressId, onQueued, onStarted, onError, onFinished, onUploaded, lastModified, creation, encryptionKey }) {
        if (!(0, utils_1.isValidFileName)(name)) {
            throw new Error(`"${name}" is not a valid file name.`);
        }
        if (onQueued) {
        ...
        try {
            ...
            const item = await new Promise((resolve, reject) => {
                const sourceStream = source instanceof stream_1.Readable ? source : stream_1.Readable.fromWeb(source);
                ...
                (0, stream_1.pipeline)(sourceStream, transformer, writeStream, err => {
                    if (err) {
                        aborted = true;
    ...
    async uploadWebFile({ file, parent, name, uuid, pauseSignal, abortSignal, onProgress, onProgressId, onQueued, onStarted, onError, onFinished, onUploaded }) {
        ...
        try {
            ...
            const fileHasher = sha512_1.sha512.create();
            if (fileSize > 0) {
                const waitForPause = async () => {
                    if (!pauseSignal || !pauseSignal.isPaused() || (abortSignal === null || abortSignal === void 0 ? void 0 : abortSignal.aborted) || aborted) {
                        return;
    ...
    async uploadLocalDirectory({ source, parent, name, pauseSignal, abortSignal, onProgress, onProgressId, onQueued, onStarted, onError, onFinished, onUploaded, onDirectoryCreated, throwOnSingleFileUploadError }) {
        ...
        try {
            ...
            if (!(0, utils_1.isValidDirectoryName)(baseDirectoryName)) {
     ...
     */
    async generateSearchItems({ name, type, uuid }) {
    ...
    async queryGlobalSearch(input) {
        ...
        const found = (await this.sdk
            .api(3)
            .search()
            .find({
            hashes: [hash]
        })).items;
        const items = (await (0, utils_1.promiseAllChunked)(found.map(async (item) => {
            if (item.type === "directory") {
                const [metadataDecrypted, metadataPathDecrypted] = await Promise.all([
                    this.sdk.getWorker().crypto.decrypt.folderMetadata({
                        metadata: item.metadata
                    }),
                    (0, utils_1.promiseAllChunked)(item.metadataPath.map(async (directoryName) => {
                        if (directoryName === "default") {
                            return directoryName;
                        }
                        return (await this.sdk.getWorker().crypto.decrypt.folderMetadata({
                            metadata: directoryName
                        })).name;
                    }))
                ]);
                return Object.assign(Object.assign({}, item), { metadataDecrypted,
            ...
            return Object.assign(Object.assign({}, item), { metadataDecrypted,
                metadataPathDecrypted });
        }))).filter(item => item.metadataDecrypted.name.length > 0 && item.metadataDecrypted.name.trim().toLowerCase().includes(inputNormalized));
     ...
     * @returns {Promise<void>}
     */
    async rebuildGlobalSearchIndex(params) {
        var _a, _b, _c;
        await this.sdk._locks.intensive.acquire();
        try {
            let added = 0;
            let generated = 0;
            (_a = params === null || params === void 0 ? void 0 : params.onProgress) === null || _a === void 0 ? void 0 : _a.call(params, {
                type: "gettingTree",
                generated,
                added,
            ...
            (_b = params === null || params === void 0 ? void 0 : params.onProgress) === null || _b === void 0 ? void 0 : _b.call(params, {
            ...
            const chunks = (0, utils_1.chunkArray)(items, 10000);
            if (chunks.length === 0) {
                return;
            }
            const addPromises = [];
            const semaphore = new semaphore_1.Semaphore(16);
            (_c = params === null || params === void 0 ? void 0 : params.onProgress) === null || _c === void 0 ? void 0 : _c.call(params, {
                type: "adding",
                generated,
                added,
                id: params.onProgressId
            ...
            await (0, utils_1.promiseAllChunked)(addPromises);
        }
        finally {
