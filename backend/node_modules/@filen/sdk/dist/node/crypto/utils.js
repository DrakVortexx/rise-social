...
exports.hashPublicLinkPassword = hashPublicLinkPassword;
...
async function generateRandomString(length = 32) {
    if (constants_1.environment === "node" || constants_1.environment === "react-native") {
...
async function generateEncryptionKey(use) {
    if (use === "file") {
        if (constants_1.FILE_ENCRYPTION_VERSION === 1 || constants_1.FILE_ENCRYPTION_VERSION === 2) {
        ...
        if (constants_1.METADATA_ENCRYPTION_VERSION === 1 || constants_1.METADATA_ENCRYPTION_VERSION === 2) {
            return await generateRandomString(32);
        }
        else {
            return await generateRandomHexString(32);
...
async function generatePrivateKeyHMAC(privateKey) {
    ...
    if (constants_1.environment === "browser" || constants_1.environment === "react-native") {
...
async function deriveKeyFromPassword({ password, salt, iterations, hash, bitLength, returnHex }) {
    ...
    else if (constants_1.environment === "react-native") {
        const result = await (0, pbkdf2_1.pbkdf2Async)(hash === "sha512" ? sha512_1.sha512 : sha256_1.sha256, password, salt, {
            dkLen: bitLength / 8,
            c: iterations
        });
        if (returnHex) {
...
async function hashPassword({ password }) {
    if (constants_1.environment === "browser" || constants_1.environment === "node" || constants_1.environment === "react-native") {
...
async function generatePasswordAndMasterKeyBasedOnAuthVersion({ rawPassword, authVersion, salt }) {
    if (authVersion === 1) {
        ...
        const derived = (await argon2id(Buffer.from(rawPassword, "utf-8"), Buffer.from(salt, "hex"), {
            t: 3,
            m: 65536,
            p: 4,
            version: 0x13,
    ...
    if (constants_1.environment === "node") {
        ...
        return new Promise((resolve, reject) => {
            node_forge_1.default.pki.rsa.generateKeyPair({
                ...
                try {
                    const publicKeyAsn1 = node_forge_1.default.pki.publicKeyToAsn1(keypair.publicKey);
                    const publicKeyDer = node_forge_1.default.asn1.toDer(publicKeyAsn1).getBytes();
                    const privateKeyAsn1 = node_forge_1.default.pki.privateKeyToAsn1(keypair.privateKey);
                    const privateKeyInfo = node_forge_1.default.pki.wrapRsaPrivateKey(privateKeyAsn1);
                    const privateKeyDer = node_forge_1.default.asn1.toDer(privateKeyInfo).getBytes();
                    resolve({
                        publicKey: node_forge_1.default.util.encode64(publicKeyDer),
                        privateKey: node_forge_1.default.util.encode64(privateKeyDer)
                    });
                }
                catch (error) {
                    reject(error);
...
async function argon2id(password, salt, options) {
    ...
    if (typeof globalThis.argon2idAsync === "function") {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        return await globalThis.argon2idAsync(password, salt, options);
    }
    return Buffer.from(await (0, argon2_1.argon2idAsync)(password, salt, options));
}
async function hashPublicLinkPassword({ password }) {
    if (constants_1.PUBLIC_LINK_VERSION === 1) {
    ...
    else if (constants_1.PUBLIC_LINK_VERSION === 2) {
    ...
    else if (constants_1.PUBLIC_LINK_VERSION === 3) {
        ...
        return {
            salt,
            hash: (await argon2id(Buffer.from(password, "utf-8"), Buffer.from(salt, "hex"), {
                t: 3,
                m: 65536,
                p: 4,
                version: 0x13,
    ...
    throw new Error(`Invalid public link version: ${constants_1.PUBLIC_LINK_VERSION}`);
}
exports.utils = {
