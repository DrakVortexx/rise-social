...
class Encrypt {
    ...
    async metadata({ metadata, key, version = constants_1.METADATA_ENCRYPTION_VERSION }) {
        ...
        if (version === 1) {
            ...
            return crypto_js_1.default.AES.encrypt(metadata, keyToUse).toString();
        }
        else if (version === 2) {
            // Old and deprecated, not in use anymore, just here for backwards compatibility
    ...
    async metadataPublic({ metadata, publicKey }) {
        if (constants_1.environment === "node") {
            ...
            const publicKeyObj = node_forge_1.default.pki.publicKeyFromPem(pemKey);
    ...
    async data({ data, key, version = constants_1.FILE_ENCRYPTION_VERSION }) {
        ...
        if (version === 1) {
            ...
            return Buffer.from(crypto_js_1.default.AES.encrypt(crypto_js_1.default.lib.WordArray.create(data), key).toString(), "base64");
        }
        else if (version === 1.5) {
            // Old and deprecated, not in use anymore, just here for backwards compatibility
            if (key.length !== 32) {
            ...
            if (constants_1.environment === "node" || constants_1.environment === "react-native") {
                const cipher = crypto_1.default.createCipheriv("aes-256-cbc", keyBytes, ivBytes);
    ...
    async dataStream({ inputFile, key, outputFile, version = constants_1.FILE_ENCRYPTION_VERSION }) {
        ...
        await new Promise((resolve, reject) => {
            (0, stream_1.pipeline)(readStream, cipher, writeStream, err => {
                if (err) {
                    reject(err);
                    return;
                }
