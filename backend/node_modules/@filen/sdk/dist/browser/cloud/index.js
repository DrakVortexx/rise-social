...
import { convertTimestampToMs, promiseAllChunked, uuidv4, normalizePath, getEveryPossibleDirectoryPath, realFileSize, promiseAllSettledChunked, isValidDirectoryName, isValidFileName, chunkArray } from "../utils";
import { environment, MAX_DOWNLOAD_THREADS, MAX_DOWNLOAD_WRITERS, MAX_UPLOAD_THREADS, DEFAULT_UPLOAD_BUCKET, DEFAULT_UPLOAD_REGION, UPLOAD_CHUNK_SIZE, MAX_CONCURRENT_DOWNLOADS, MAX_CONCURRENT_UPLOADS, MAX_CONCURRENT_DIRECTORY_DOWNLOADS, MAX_CONCURRENT_DIRECTORY_UPLOADS, MAX_CONCURRENT_SHARES, BUFFER_SIZE, FILE_ENCRYPTION_VERSION } from "../constants";
...
export class Cloud {
        ...
        for (const folder of contents.folders) {
            try {
                ...
                const folderPath = folder.parent === "base"
                    ? "/"
                    : pathModule.posix.join(parentPath, decrypted.name.length > 0 ? decrypted.name : `CANNOT_DECRYPT_NAME_${folder.uuid}`);
        ...
        for (const file of contents.files) {
            promises.push(new Promise((resolve, reject) => {
                ...
                decryptPromise
                    ...
                    const filePath = pathModule.posix.join(parentPath, decrypted.name.length > 0 ? decrypted.name : `CANNOT_DECRYPT_NAME_${file.uuid}`);
    ...
    async downloadDirectoryToLocal({ uuid, type = "normal", linkUUID, linkHasPassword, linkPassword, linkSalt, to, abortSignal, pauseSignal, onQueued, onStarted, onError, onFinished, onProgress, onProgressId, linkKey }) {
        ...
        try {
            ...
            for (const path in tree) {
                ...
                const resolvedDestination = pathModule.resolve(destinationPath);
                const resolvedFilePath = pathModule.resolve(filePath);
                if (resolvedFilePath !== resolvedDestination && !resolvedFilePath.startsWith(resolvedDestination + pathModule.sep)) {
                    continue;
                }
                promises.push(new Promise((resolve, reject) => {
    ...
    async queryGlobalSearch(input) {
        ...
        const items = (await promiseAllChunked(found.map(async (item) => {
            if (item.type === "directory") {
                const [metadataDecrypted, metadataPathDecrypted] = await Promise.all([
                    this.sdk.getWorker().crypto.decrypt.folderMetadata({
                        metadata: item.metadata
                    }),
                    promiseAllChunked(item.metadataPath.map(async (directoryName) => {
