...
export class Encrypt {
     ...
     */
    async metadata({ metadata, key, version = METADATA_ENCRYPTION_VERSION }) {
    ...
    async data({ data, key, version = FILE_ENCRYPTION_VERSION }) {
        if (key.length === 0) {
            throw new Error("Invalid key.");
        }
        // If the key provided is not a 64 char hex encoded key, we cannot use it with v3. Downgrade to v2.
    ...
    async dataStream({ inputFile, key, outputFile, version = FILE_ENCRYPTION_VERSION }) {
        if (key.length === 0) {
            throw new Error("Invalid key.");
        }
        if (environment !== "node") {
